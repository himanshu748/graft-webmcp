...
// and its inlined runtime drives every read-only tool against this live DOM, so
// none of them need handler code here. What remains is the one write action the
// review gate held, which the owner bound by hand on purpose.
...
// Graft held this write candidate for review. The owner read it, decided it was
// correct, and bound a real handler. That decision is the whole point of the
// review gate: Graft proposes, a human ships.
...
async function boot() {
  // No handler map: the read-only contracts run on the bundled runtime.
  const report = await registerGraftTools();
  const adapterCount = report.registered.length;
  ...
  status.textContent =
    live > 0
      ? `${live} WebMCP tools live: ${report.registered.join(", ")}. ${adapterCount} registered from the Graft adapter${writeRegistered ? ", plus 1 owner-bound write" : ""}.`
