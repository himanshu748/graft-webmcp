...
That site was not compiled by Graft at runtime. It ships `graft-adapter.js`, generated by Graft's export from the Signal Cabinet page. Its five read-only tools need no handler code at all: the inlined runtime resolves the same selectors the compiler scored against the live DOM. Regenerate the adapter with `npx vite-node scripts/build-owner-adapter.ts`.
